...
let path = {
  ...
  source: {
    ...
    fonts: `${sourceFolder}/fonts/**/*.{woff2,woff,ttf,otf}`,
  },
...
function getFonts() {
  src(path.source.fonts).pipe(ttf2woff()).pipe(dest(path.dest.fonts));
  return src(path.source.fonts).pipe(ttf2woff2()).pipe(dest(path.dest.fonts));
}
